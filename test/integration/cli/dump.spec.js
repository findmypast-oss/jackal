...
describe('CLI.Dump Integration Test', function () {
  ...
  context('using the pretty reporter', function () {
    ...
    it('should get an up to date copy of the database serialised as JSON and pretty printed', function (done) {
      exec(`node index dump -r pretty http://localhost:${port}`, {}, (err, stdout, stderr) => {
        expect(err).to.not.exist
        expect(stdout.startsWith('filename:           test/integration/api/db.json')).to.be.true
        expect(stderr).to.equal('')
        done()
      })
    })
