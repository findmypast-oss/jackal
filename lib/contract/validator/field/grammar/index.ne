joi                 -> "Joi." type ( dot globalModifier ):*

type                -> booleanType | dateType | numberType | stringType

booleanType         -> ( "boolean()" | booleanType dot booleanTruthy | booleanType dot booleanFalsy ) ( dot booleanInsensitive ):?
...
dateType            -> "date()" ( dot dateMin ):? ( dot dateMax ):? ( dot dateIso ):? ( dot dateTimestamp ):?
...
numberType          -> "number()" ( dot numMin ):? ( dot numMax ):? ( dot numGreater ):? ( dot numLess ):? ( dot numInteger ):? ( dot numPrecision ):? ( dot numMultiple ):? ( dot numPositive ):? ( dot numNegative ):?
...
stringType          -> "string()" ( dot strInsensitive ):? ( dot strMin ):? ( dot strMax ):? ( dot strTruncate ):? ( dot strCreditCard ):? ( dot strLength ):? ( dot strRegex ):? ( dot strReplace ):? ( dot strAlphaNum ):? ( dot strToken ):? ( dot strEmail ):? ( dot strIp ):? ( dot strUri ):? ( dot strGuid ):? ( dot strHex ):? ( dot strBase64 ):? ( dot strHostname ):? ( dot strLowerCase ):? ( dot strUpperCase ):? ( dot strTrim ):? ( dot strIsoDate ):?
...
globalModifier      -> ( dot allow ):? ( dot valid ):? ( dot invalid ):? ( dot required ):? ( dot optional ):? ( dot forbidden ):?
