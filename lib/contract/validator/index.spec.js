...
test('validate', t => {
  ...
  const contract = {}
  let expected = { valid: false, errors: [ { name: 'ContractValidationError', message: '\t"name" is required' } ] }
  t.same(validate(contract), expected)

  contract.name = 123
  expected = { valid: false, errors: [ { name: 'ContractValidationError', message: '\t"name" must be a string' } ] }
  t.same(validate(contract), expected)

  contract.name = 'name'
  expected = { valid: false, errors: [ { name: 'ContractValidationError', message: '\t"consumer" is required' } ] }
  t.same(validate(contract), expected)

  contract.consumer = 123
  expected = { valid: false, errors: [ { name: 'ContractValidationError', message: '\t"consumer" must be a string' } ] }
  t.same(validate(contract), expected)
