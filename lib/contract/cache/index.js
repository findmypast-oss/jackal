...
const insertForConsumer = function (coll, hash, contracts) {
  const collection = getCollection(coll)
  const consumer = contracts[0].consumer
  removeDuplicateConsumerContracts(collection, consumer)
  const doc = { hash: hash, consumer: consumer, contracts: contracts }
...
const insertForProviders = function (contracts) {
  return contracts.every(function (contract) {
    ...
    if (!cached(provider, hash)) {
      ...
      removeDuplicateProviderContract(collection, name, contract.consumer)
      const doc = {
        hash: hash,
        name: name,
...
const removeDuplicateConsumerContracts = function (collection, consumer) {
  const query = { 'consumer' : { '$eq' : consumer } }
  collection.findAndRemove(query)
}

const removeDuplicateProviderContract = function (collection, name, consumer) {
