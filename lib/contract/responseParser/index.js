...
const strictEval = require('../../strictEvalFork')
...
const parseResponse = response => {
  ...
  if (response.body) {
    if (Array.isArray(response.body)) {
      ...
      parsed.body = strictEval(`const Joi = require('joi'); ${response.body}`)
    } else {
...
const parseArray = array => {
  const joiSpec = array.map(item => {
    if (Array.isArray(item)) {
      ...
      return strictEval(`const Joi = require('joi'); ${item}`)
    } else {
      return item
    }
  })
...
const parseObject = object => {
  ...
  const joiSpec = keys.map(key => {
    if (Array.isArray(object[key])) {
      ...
      return strictEval(`const Joi = require('joi'); ${object[key]}`)
    } else {
      return object[key]
    }
  })
