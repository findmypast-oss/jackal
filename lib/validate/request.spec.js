...
describe('validateRequest', function (t) {
  ...
  it('should fail when given a request object with non-string baseUrl', function () {
    request.baseUrl = 123
    expected = { valid: false, error: { name: 'ValidationError', message: '"baseUrl" must be a string' } }
    expect(validateRequest(request)).to.be.deep.equal(expected)
  })

  it('should fail when given a request object with invalid baseUrl', function () {
    request.baseUrl = 'string'
    expected = { valid: false, error: { name: 'ValidationError', message: '"baseUrl" must be a valid uri' } }
    expect(validateRequest(request)).to.be.deep.equal(expected)
  })

  it('should fail when given a request object with valid baseUrl and non-string method', function () {
    request.baseUrl = 'http://www.findmypast.co.uk'
    request.method = 123
