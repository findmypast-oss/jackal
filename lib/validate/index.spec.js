...
describe('validate', function () {
  ...
  it('should fail with an empty request property', function () {
    contract.request = {}
    expected = { contract: 'provider/api/scenario <- consumer', valid: false, errors: [ { name: 'ContractValidationError', message: '"response" is required' }, { name: 'RequestValidationError', message: '"baseUrl" is required' } ] }
    expect(validate(contract, consumer, provider, api, scenario)).to.be.deep.equal(expected)
  })

  it('should fail with an invalid request baseUrl', function () {
    contract.request.baseUrl = 'baseUrl'
    expected = { contract: 'provider/api/scenario <- consumer', valid: false, errors: [ { name: 'ContractValidationError', message: '"response" is required' }, { name: 'RequestValidationError', message: '"baseUrl" must be a valid uri' } ] }
    expect(validate(contract, consumer, provider, api, scenario)).to.be.deep.equal(expected)
  })

  it('should fail with a valid request property, valid consumer property and valid name property only', function () {
    contract.request.baseUrl = 'http://www.google.com'
